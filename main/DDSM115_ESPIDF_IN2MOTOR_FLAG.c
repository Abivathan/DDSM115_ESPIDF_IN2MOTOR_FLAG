...
void currentLoopCommand(uint8_t motorID1, float current1, uint8_t motorID2, float current2) {
    // Command for Motor 1
    uint16_t currentData1 = (uint16_t)((current1 + 8.0) / 16.0 * 65535);
    ...
    // Command for Motor 2
    uint16_t currentData2 = (uint16_t)((current2 + 8.0) / 16.0 * 65535);
...
void velocityLoopCommand(uint8_t motorID1, float velocity1, uint8_t motorID2, float velocity2) {
    // Command for Motor 1
    int16_t velocityData1 = (int16_t)(velocity1 * 65535 / 660.0);
    uint8_t command1[10] = {motorID1, 0x64, (uint8_t)(velocityData1 >> 8), (uint8_t)velocityData1, 0x00, 0x00, 0x00, 0x00, 0x00};
    command1[9] = calculateCRC8(command1, 9);
    sendCommand(command1, sizeof(command1));
    ...
    // Command for Motor 2
    int16_t velocityData2 = (int16_t)(velocity2 * 65535 / 660.0);
    uint8_t command2[10] = {motorID2, 0x64, (uint8_t)(velocityData2 >> 8), (uint8_t)velocityData2, 0x00, 0x00, 0x00, 0x00, 0x00};
    command2[9] = calculateCRC8(command2, 9);
    sendCommand(command2, sizeof(command2));
...
void positionLoopCommand(uint8_t motorID1, float position1, uint8_t motorID2, float position2) {
    // Command for Motor 1
    uint16_t positionData1 = (uint16_t)(position1 * 65535 / 360.0);
    ...
    // Command for Motor 2
    uint16_t positionData2 = (uint16_t)(position2 * 65535 / 360.0);
